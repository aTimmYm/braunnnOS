...
local function Textfield_draw(self)
	...
	if self.root.focus ~= self and #self.text == 0 and #self.hint <= self.w then
		...
		screen.write(_sub(text, self.offset + 1, _min(#self.text, self.offset + self.w)).._rep(" ", self.w - #self.text + self.offset), self.x, self.y, self.color_bg, self.color_txt)
	end
end
...
local function TabBar_draw(self)
	...
	screen.draw_rectangle(self.x, self.y, self.w, self.h, self_color_bg)
	local offset = 1
